...
- 结论：严格确定性口径下性能可恢复到接近旧水平（E040达69.59/71.29），确认掉点是轨迹抽样差异而非系统性损失；但三seed方差很大（约±4.0 mAP），且最佳epoch分布极不稳定（4/20/24），单次运行的最佳点选择敏感。后续口径保持严格确定性；性能表述必须用三seed均值，论文主表不得用单次结果。曲线高震荡提示可考虑延长训练或改进验证/最佳点选择策略，属待决项。
