...
- 结论：严格确定性机制本身工作正常且可复现，但确定化后同配置性能明显下降且逐epoch验证曲线波动加剧（45–62区间）。因此：1) 修复前的旧结果（含E031/E036）不能直接作为确定性口径下的性能参考；2) 确定性口径下的可比基线应以E038为准；3) 是否恢复cuDNN benchmark等非严格设置换取性能，需用户决策后再定，在此之前不启动新的性能对比运行。
