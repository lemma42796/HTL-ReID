...
- 完成结果：returncode 0，墙钟793.0秒（约13分13秒），完整50 epoch正常完成，无残留训练进程；输出目录432 MB，最佳checkpoint约432 MB，`DONE`、结果JSON、配置快照和日志均已保留。
- 最佳结果：epoch 4，mAP 61.31%、Rank-1 62.68%、Rank-5 75.24%、Rank-10 83.61%。epoch 50为59.66%/63.04%/75.72%/82.78%。
- 与E031比较：墙钟由895.4秒降至793.0秒，缩短102.4秒（约11.4%）；mAP下降10.23、Rank-1下降12.56、Rank-5下降8.61、Rank-10下降3.11个百分点。相对E032的793.6秒只快0.6秒，说明batch 96增至128几乎没有进一步缩短本次端到端墙钟。
- 结论：batch 128在32 GB GPU上能够完成，但保持E031学习率与50-epoch日程时精度显著下降，未达到71.54 mAP / 75.24 Rank-1门槛，不能直接固定为后续协议。该结果相对E031只改变batch，因此证明当前优化日程不能直接从batch 64迁移到128；若坚持batch 128，必须重新调整学习率或按优化步数重设训练日程后再验证。
