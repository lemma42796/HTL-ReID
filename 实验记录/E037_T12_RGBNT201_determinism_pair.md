...
- 训练代码commit：`46395b1`
- 对应实验：DET-A / DET-B
- 实验目的：针对E031/E036同seed曲线分叉建立受控的确定性训练机制，并用两次完全相同的CUDA训练验证采样顺序、T12重建目标、loss和评估指标能否逐位重复。本实验不以复现E031最高性能为目标。
...
- 结论：严格确定性机制校验通过，已证明同一固定配置与seed的训练过程能够逐位重复。该结论只涉及过程确定性，不表示E031的71.54% mAP / 75.24% Rank-1最高性能已经复现；本测试不用于论文精度结果，不触发消融。
