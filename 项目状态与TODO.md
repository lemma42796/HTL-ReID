...
- E036/T12-B64-REC25正在运行：训练commit `33b006b`，runner PID 12047、timeout PID 12049、主进程PID 12050。已恢复E031的batch 64、base LR 3.5e-4、backbone LR 2.8e-4与50-epoch cosine horizon，仅在epoch 25提前停止；epoch 1训练正常完成，每epoch 54 iterations，首轮warm-up LR 5.95e-5与E031一致，启动显存约5,290 MiB，无OOM/NaN。目标是用约7–8分钟复现E031的71.54 mAP / 75.24 Rank-1。
...
2. E035已把batch 128结果恢复到68.23 mAP / 73.68 Rank-1并将时间减半，但仍低于门槛3.31/1.56；下一步优先在5e-4至7e-4之间做一次LR单变量筛选，不增加训练epoch。若LR微调仍不足，再实现物理batch 128下的虚拟batch 64 Triplet难样本分组；
3. batch 128只有同时达到稳定性与精度门槛后才能用于后续RGBNT201模型筛选；当前尚未通过，不得仅凭显存可运行而固定；
